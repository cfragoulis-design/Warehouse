-- Grant the explicitly configured restricted Warehouse runtime role only the
-- privileges required by the label-layout control plane. The migration runner
-- installs warehouse.runtime_role as a transaction-local setting after exact
-- operator confirmation. Broad/default role grants are forbidden.
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'warehouse.runtime_role can assume an elevated database role';
    END IF;
    ...
    IF versions_rel IS NULL OR active_rel IS NULL THEN
        RAISE EXCEPTION 'label-layout relations are missing';
    END IF;
    ...
    -- Includes direct ownership and indirect membership in an owner role.
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_class AS relation
        ...
        RAISE EXCEPTION
            'warehouse.runtime_role owns or can assume ownership of label-layout objects';
    END IF;
    ...
    -- Fail closed if rights inherited from PUBLIC or another role make this
    -- runtime identity broader than the reviewed application contract.
    IF NOT pg_catalog.has_table_privilege(
        runtime_role, versions_rel, 'SELECT'
    ) OR NOT pg_catalog.has_table_privilege(
        runtime_role, active_rel, 'SELECT'
    ) THEN
        RAISE EXCEPTION 'warehouse.runtime_role is missing label-layout read access';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'warehouse.runtime_role has invalid label-layout version insert access';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'warehouse.runtime_role has invalid label-layout active update access';
    END IF;

    IF EXISTS (
        SELECT 1
        FROM unnest(
            ARRAY['DELETE', 'TRUNCATE', 'TRIGGER', 'MAINTAIN']
        ...
        RAISE EXCEPTION 'warehouse.runtime_role has forbidden label-layout access';
    END IF;
    ...
    IF NOT pg_catalog.has_sequence_privilege(
        ...
        RAISE EXCEPTION 'warehouse.runtime_role has invalid label-layout sequence access';
    END IF;
    ...
    IF pg_catalog.has_table_privilege(
        ...
        RAISE EXCEPTION
            'warehouse.runtime_role must not hold label-layout grant options';
    END IF;
END
$migration$;
