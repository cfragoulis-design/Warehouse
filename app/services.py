...
async def stock_adjust(
    ...
    if wants_json:
        ...
        if min_stock > 0:
            if c_qty <= 0:
                low_level = "critical"
            elif c_qty < min_stock:
                low_level = "low"

        missing_qty = Decimal(getattr(p, "missing_qty", 0) or 0)
        ...
        return JSONResponse(
            {
                ...
                "low_level": low_level,
                "missing_text": fmtqty(missing_qty, p.unit),
                "missing_value": float(missing_qty or 0),
            }
        )
...
async def stock_fulfill_pending(
    ...
    """Fulfill Pending (Target - Central) from WORKSHOP to CENTRAL.

    Rule (agreed):
    - When user presses Pending, we attempt a transfer from WORKSHOP to CENTRAL.
    - If WORKSHOP has less than requested, we transfer what exists (partial).
    - The shortfall is recorded in Product.missing_qty (if column exists).
    - Target is reset to 0 to close the Pending request.
    """
    if user.role not in ("admin", "workshop"):
        raise HTTPException(status_code=403, detail="Forbidden")
    ...
    # partial transfer
    send_qty = pending if ws_qty >= pending else ws_qty
    if send_qty <= 0:
        # Still close pending request (target=0), but record full missing if supported.
        missing_col = getattr(Product, "missing_qty", None)
        if missing_col is not None:
            current_missing = Decimal(getattr(p, "missing_qty", 0) or 0)
            setattr(p, "missing_qty", current_missing + pending)
    ...
    # record missing if partial
    if send_qty < pending:
        missing_col = getattr(Product, "missing_qty", None)
        if missing_col is not None:
            current_missing = Decimal(getattr(p, "missing_qty", 0) or 0)
            setattr(p, "missing_qty", current_missing + (pending - send_qty))

    # close pending request by resetting target
    p.target_central = Decimal("0")
    db.commit()
    return RedirectResponse(url="/stock", status_code=303)
...
async def stock_send_missing(
    product_id: int = Form(...),
    db: Session = Depends(get_db),
    user: User = Depends(require_login),
):
    """Send Missing from WORKSHOP to CENTRAL without touching Target/Pending."""
    if user.role not in ("admin", "workshop"):
        raise HTTPException(status_code=403, detail="Forbidden")
    ...
    # If missing column not present, nothing to do.
    if getattr(Product, "missing_qty", None) is None:
        return RedirectResponse(url="/stock", status_code=303)

    missing = Decimal(getattr(p, "missing_qty", 0) or 0)
    if missing <= 0:
        return RedirectResponse(url="/stock", status_code=303)
    ...
    ws_qty = get_stock_qty(db, product_id, workshop.id)
    send_qty = missing if ws_qty >= missing else ws_qty
    if send_qty <= 0:
        return RedirectResponse(url="/stock", status_code=303)
    ...
    setattr(p, "missing_qty", missing - send_qty)
    db.commit()
    return RedirectResponse(url="/stock", status_code=303)
